...
class Monke {
  ...
  calculateWorryLevel(tempItem) {
    if (this.addOrMultiply === "+") return Math.floor(tempItem + this.value);
    else return Math.floor(tempItem * this.value);
  }
  ...
  renderRound() {
    ...
    while (this.items.length) {
      ...
      let worryLevel;
      if (this.value == "old") worryLevel = Math.floor(tempItem * tempItem);
      else worryLevel = this.calculateWorryLevel(tempItem);
      worryLevel %= grandModulus;
...
const parseInput = () => {
  for (let i = 0; i < lines.length; i += 6) {
    ...
    grandModulus *= JSON.parse(lines[i + 3]);
...
let monkeArray = [];
let grandModulus = 1; // jak cos, to ja tego nie wymyslilem. ide zajarac
parseInput();
